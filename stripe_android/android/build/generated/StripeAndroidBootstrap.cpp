...
#define TAG "com.ravindra.stripe"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> StripeAndroid_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("StripeAndroid.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::StripeAndroidBindings::lookupGeneratedInit(
...
static void StripeAndroid_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(StripeAndroid_getBinding)->GetFunction());
}

static void StripeAndroid_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::StripeAndroidBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry StripeAndroidBinding = {
	"com.ravindra.stripe",
	StripeAndroid_init,
	StripeAndroid_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_ravindra_stripe_StripeAndroidBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.ravindra.stripe", &StripeAndroidBinding);
	titanium::KrollBindings::addExternalLookup(&(::StripeAndroidBindings::lookupGeneratedInit));
}
